...
(function(){
    ...
    if ($('table.GridTableContent')){
        var tr = $('tr[bgcolor]');
        var tr2 = $('tr[bgcolor],.GridDoubleRow,.GridSingleRow');
        tr2.each(function(){
    ...
    function getInfo(){
        ...
        var trselected = selectBox(tr2);
        info += trselected.map(getInfoInEach).get().join('');
        var txtname = $(tr2).find('a.fz14,.author_flag,.GridRightColumn').find('font.Mark').eq(0).text().trim() + '.txt';
    ...
    function downLoad(){
        ...
        trselected.each(function(){
            var btdown=$(this).find('a.briefDl_D,.briefDl_Y')[0];
    ...
    function getInfoInEach(index){
        var pnum = index + 1;
        var ptime = $(this).find('td:eq(4),label:contains(发表时间)').text().replace('发表时间：','').trim().substr(0,10);
        var pname = $(this).find('.author_flag,.author').text().split(';')[0].replace('，','').trim().split('  ').pop();
        pname = pname.length > 15 ? pname.split(' ')[1] : pname;
        var ptitle = $(this).find('a.fz14,h3.title_c a').text().trim().replace(/[\/:*?"<>|]+/g,'_');
